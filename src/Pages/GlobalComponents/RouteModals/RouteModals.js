...
export default function RouteModals({ leftPane = null, rightPane = null, name = null, setModalsSettings }) {
  ...
  function spanLooper(char, num) {
    let _ = []
    for (let i = 0; i <= num; i++) {
      _.push((<span className={currentStyle.themeText}>{char}</span>))
    }
    return _
  }
        ...
        <div className={classes.contentRest}>
          ...
          <button
            className={classify(
              classes.sendButton,
              currentStyle.altbg,
              currentStyle.hoverTerbg,
              currentStyle.themeText,
              currentStyle.hoverThemeText)}
            onClick={() => window.open(leftPane, '_blank')}>
            Live Site
          </button>
        </div>
        <button
          className={classify(
            ...
            currentStyle.hoverTerbg,
            currentStyle.hoverThemeText)}
          onClick={() => setModalsSettings({ active: false, leftUrl: null, rightUrl: null })}>{spanLooper('<', 5)}
        </button>
      </div>
      ...
      <div className={classify(classes.rightToRepo, (rightPane != null && classes.active), currentStyle.secondarybg)} >
        <button
          className={classify(
            ...
            currentStyle.hoverTerbg,
            currentStyle.hoverThemeText)}
          onClick={() => setModalsSettings({ active: false, leftUrl: null, rightUrl: null })}>
            {spanLooper('>', 5)}
        </button>
        <div className={classes.contentRest}>
